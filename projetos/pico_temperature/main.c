...
#include "ssd1306.h"
#include "adc.h" // Inclui o cabeçalho com a função adc_to_celsius
...
// Função para ler a temperatura usando a função adc_to_celsius
float read_temperature() {
    uint16_t raw = adc_read(); // Lê o valor bruto do ADC (0 a 4095)
    return adc_to_celsius(raw); // Converte o valor bruto para temperatura em Celsius
}
